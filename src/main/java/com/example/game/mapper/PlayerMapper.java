...
public interface PlayerMapper {
    ...
    void initSkillLv(Integer playerId);

    @Insert("insert ignore into skill_equip (player_id,slot_index, skill_id) select #{playerId},`index`,null from index_skill")
    void initSkillSlot(Integer playerId);

    @Insert("insert ignore into medicine_number (player_id, medicine_id, number) select #{playerId},id,5 from medicine;")
    void initMedicineNumber(Integer playerId);
    //
